...
if(!function_exists('mbw_update_check')){
	function mbw_update_check(){
		...
		if(version_compare($current_db_version, $mb_db_version, "<")){
			...
			if(version_compare($current_db_version, '1.0.7', "<")){		//1.0.6 버젼 업데이트
				$options_rows['anti_spam_protection']		= "('setup', 'board', 'W_ANTI_SPAM', 'anti_spam_protection', '1', '1,0', 'W_ON_OFF', '', '', '', '', 'radio', '')";
